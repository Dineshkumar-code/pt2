...
import platform
setpoint = 5
directory =  r'/home/pi/Desktop/tempfolder'
    ...
    try:
      ...
      try:
        while True:
          ...
          os.chdir(directory)
          #print("Minimum Temprature " + str(minTemp))
          #print("Maximum Temprature " + str(maxTemp))
          if maxTemp > (minTemp + setpoint):
                    prev_line = "datetime : " +str(datetime.datetime.now()) +" :  "+ "Temp_raised to " +" : " + str(maxTemp) + "°F"
                    f = open(text_dir,"a+")
                    f.write(prev_line + '\n')
                    print(prev_line)
                    imgrgb = cv2.cvtColor(img,cv2.COLOR_BGR2HSV)
                    cv2.imwrite('sample.jpg',imgrgb)
          cv2.waitKey(1)
